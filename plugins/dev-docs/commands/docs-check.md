...
For each authored doc in docs/overview/*.md (ignore docs/overview/decisions/):
...
3. If a design decision was made on this branch that isn't recorded, propose a
   decision record (see docs/overview/decisions/README.md) and ask before
   writing it.
...
If the diff clearly makes some other doc elsewhere in docs/ (outside
docs/overview/) wrong, mention it in one line. Don't propose an edit for it —
that doc isn't managed by this command.
